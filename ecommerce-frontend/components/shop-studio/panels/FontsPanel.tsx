...
export default function FontsPanel({ 
  ...
  // ==================== POUR LE BLOC BANNER - QUAND ON CLIQUE SUR LE FOND ====================
  if ((isBanner || isCarouselBanner) && target === 'background') {
    return (
      <div className="space-y-4">
        <div className="bg-gray-800/50 rounded-lg p-3 space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => onUpdateBlock(selectedBlock.id, { showTitle: selectedBlock.props?.showTitle === false ? true : false })}
              className={`px-3 py-1 rounded text-xs ${selectedBlock.props?.showTitle !== false ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}
            >
              {selectedBlock.props?.showTitle !== false ? '✓ Activé' : '✗ Désactivé'}
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => onUpdateBlock(selectedBlock.id, { showSubtitle: selectedBlock.props?.showSubtitle === false ? true : false })}
              className={`px-3 py-1 rounded text-xs ${selectedBlock.props?.showSubtitle !== false ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}
            >
              {selectedBlock.props?.showSubtitle !== false ? '✓ Activé' : '✗ Désactivé'}
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => onUpdateBlock(selectedBlock.id, { showButton: selectedBlock.props?.showButton === false ? true : false })}
              className={`px-3 py-1 rounded text-xs ${selectedBlock.props?.showButton !== false ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}
            >
              {selectedBlock.props?.showButton !== false ? '✓ Activé' : '✗ Désactivé'}
            </button>
          </div>
        </div>
  ...
  // ==================== POUR LE BLOC SCREEN-BANNER - QUAND ON CLIQUE SUR LE FOND ====================
  ...
  // ==================== POUR LE BLOC BANNER - QUAND ON CLIQUE SUR LE TEXTE ====================
      ...
      <div className="space-y-4">
        ...
        {hasAnyTextVisible() && (
          <>
            ...
            {activeTab === 'fonts' && (
              <>
                {selectedBlock.props?.showTitle !== false && (
                  <>
                    <div><label className="text-xs text-gray-400 block mb-1">Police du titre</label><select value={selectedBlock.props?.titleFont || 'Poppins'} onChange={(e) => onUpdateBlock(selectedBlock.id, { titleFont: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs">{fonts.map(font => (<option key={font} value={font}>{font}</option>))}</select></div>
                    <div><label className="text-xs text-gray-400 block mb-1">Taille du titre: {selectedBlock.props?.titleFontSize || 48}px</label><input type="range" min="24" max="96" step="1" value={selectedBlock.props?.titleFontSize || 48} onChange={(e) => onUpdateBlock(selectedBlock.id, { titleFontSize: parseInt(e.target.value) })} className="w-full" /></div>
                    <div><label className="text-xs text-gray-400 block mb-1">Poids du titre</label><select value={selectedBlock.props?.titleFontWeight || '700'} onChange={(e) => onUpdateBlock(selectedBlock.id, { titleFontWeight: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs">{fontWeights.map(w => (<option key={w.value} value={w.value}>{w.label}</option>))}</select></div>
                  </>
                )}
                ...
                {selectedBlock.props?.showSubtitle !== false && (
                  <div className="border-t border-gray-700 pt-3 mt-2">
                    ...
                    <div><label className="text-xs text-gray-400 block mb-1">Police du sous-titre</label><select value={selectedBlock.props?.subtitleFont || 'Inter'} onChange={(e) => onUpdateBlock(selectedBlock.id, { subtitleFont: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs">{fonts.map(font => (<option key={font} value={font}>{font}</option>))}</select></div>
                    <div className="mt-2"><label className="text-xs text-gray-400 block mb-1">Taille du sous-titre: {selectedBlock.props?.subtitleFontSize || 18}px</label><input type="range" min="12" max="48" step="1" value={selectedBlock.props?.subtitleFontSize || 18} onChange={(e) => onUpdateBlock(selectedBlock.id, { subtitleFontSize: parseInt(e.target.value) })} className="w-full" /></div>
                    <div className="mt-2"><label className="text-xs text-gray-400 block mb-1">Poids du sous-titre</label><select value={selectedBlock.props?.subtitleFontWeight || '400'} onChange={(e) => onUpdateBlock(selectedBlock.id, { subtitleFontWeight: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs">{fontWeights.map(w => (<option key={w.value} value={w.value}>{w.label}</option>))}</select></div>
                  </div>
                )}
                ...
                {selectedBlock.props?.showButton !== false && (
                  <div className="border-t border-gray-700 pt-3 mt-2">
                    ...
                    <div><label className="text-xs text-gray-400 block mb-1">Police du bouton</label><select value={selectedBlock.props?.buttonFont || 'Inter'} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonFont: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs">{fonts.map(font => (<option key={font} value={font}>{font}</option>))}</select></div>
                    <div className="mt-2"><label className="text-xs text-gray-400 block mb-1">Taille du bouton: {selectedBlock.props?.buttonFontSize || 16}px</label><input type="range" min="12" max="32" step="1" value={selectedBlock.props?.buttonFontSize || 16} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonFontSize: parseInt(e.target.value) })} className="w-full" /></div>
                    <div className="mt-2"><label className="text-xs text-gray-400 block mb-1">Poids du bouton</label><select value={selectedBlock.props?.buttonFontWeight || '500'} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonFontWeight: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs">{fontWeights.map(w => (<option key={w.value} value={w.value}>{w.label}</option>))}</select></div>
                  </div>
                )}
              </>
            )}
            ...
            {activeTab === 'text-effects' && (
              <>
                {selectedBlock.props?.showTitle !== false && (
                  <>
                    <div><label className="text-xs text-gray-400 block mb-1">Couleur du titre</label><div className="flex items-center gap-2"><input type="color" value={selectedBlock.props?.titleColor || '#ffffff'} onChange={(e) => onUpdateBlock(selectedBlock.id, { titleColor: e.target.value })} className="w-8 h-8 rounded border-0 cursor-pointer" /><input type="text" value={selectedBlock.props?.titleColor || '#ffffff'} onChange={(e) => onUpdateBlock(selectedBlock.id, { titleColor: e.target.value })} className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs font-mono" /></div></div>
                    <div><label className="text-xs text-gray-400 block mb-1">Dégradé pour le titre</label><div className="grid grid-cols-3 gap-1">{['linear-gradient(135deg, #667eea 0%, #764ba2 100%)','linear-gradient(135deg, #f093fb 0%, #f5576c 100%)','linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)','linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)','linear-gradient(135deg, #fa709a 0%, #fee140 100%)','linear-gradient(135deg, #0f2027 0%, #203a43 50%, #2c5364 100%)'].map((grad, idx) => (<button key={idx} onClick={() => onUpdateBlock(selectedBlock.id, { titleGradient: grad, titleColor: null })} className={`h-8 rounded border transition-all hover:scale-105 ${selectedBlock.props?.titleGradient === grad ? 'border-primary ring-1 ring-primary' : 'border-gray-700'}`} style={{ background: grad }} title={`Dégradé ${idx + 1}`} />))}</div>{selectedBlock.props?.titleGradient && <button onClick={() => onUpdateBlock(selectedBlock.id, { titleGradient: null, titleColor: '#ffffff' })} className="w-full mt-2 text-xs text-gray-400 hover:text-white">✕ Supprimer le dégradé</button>}</div>
                  </>
                )}

                {selectedBlock.props?.showSubtitle !== false && (
                  <div className="border-t border-gray-700 pt-3 mt-2"><label className="text-xs text-gray-400 block mb-1">Couleur du sous-titre</label><div className="flex items-center gap-2"><input type="color" value={selectedBlock.props?.subtitleColor || '#ffffff'} onChange={(e) => onUpdateBlock(selectedBlock.id, { subtitleColor: e.target.value })} className="w-8 h-8 rounded border-0 cursor-pointer" /><input type="text" value={selectedBlock.props?.subtitleColor || '#ffffff'} onChange={(e) => onUpdateBlock(selectedBlock.id, { subtitleColor: e.target.value })} className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs font-mono" /></div></div>
                )}

                {selectedBlock.props?.showButton !== false && (
                  <>
                    <div className="border-t border-gray-700 pt-3 mt-2"><label className="text-xs text-gray-400 block mb-1">Couleur du bouton</label><div className="flex items-center gap-2"><input type="color" value={selectedBlock.props?.buttonBackgroundColor || '#2563EB'} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonBackgroundColor: e.target.value })} className="w-8 h-8 rounded border-0 cursor-pointer" /><input type="text" value={selectedBlock.props?.buttonBackgroundColor || '#2563EB'} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonBackgroundColor: e.target.value })} className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs font-mono" /></div></div>
                    <div><label className="text-xs text-gray-400 block mb-1">Texte du bouton</label><div className="flex items-center gap-2"><input type="color" value={selectedBlock.props?.buttonTextColor || '#ffffff'} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonTextColor: e.target.value })} className="w-8 h-8 rounded border-0 cursor-pointer" /><input type="text" value={selectedBlock.props?.buttonTextColor || '#ffffff'} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonTextColor: e.target.value })} className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-white text-xs font-mono" /></div></div>
                    <div><label className="text-xs text-gray-400 block mb-1">Arrondi du bouton: {selectedBlock.props?.buttonBorderRadius || 8}px</label><input type="range" min="0" max="50" value={selectedBlock.props?.buttonBorderRadius || 8} onChange={(e) => onUpdateBlock(selectedBlock.id, { buttonBorderRadius: parseInt(e.target.value) })} className="w-full" /></div>
                  </>
                )}
          ...
          <>
            ...
            {activeTab === 'text-effects' && (
              <>
                ...
                <div><label className="text-xs text-gray-400 block mb-1">Contour du texte: {selectedBlock.props?.textStrokeWidth || 0}px</label><input type="range" min="0" max="5" step="0.5" value={selectedBlock.props?.textStrokeWidth || 0} onChange={(e) => onUpdateBlock(selectedBlock.id, { textStrokeWidth: parseFloat(e.target.value) })} className="w-full" /></div>
